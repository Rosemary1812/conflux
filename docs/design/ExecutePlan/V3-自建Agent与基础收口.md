...
| G2 | 单聊中 `/skill-creator` 走多轮对话式生成 Skill；用户描述目标，LLM 生成 Skill 草稿，预览确认后保存 | `skills` 表新增一条；该 Skill 出现在 `/` 命令面板与后续 `/agent-creator` 的"引用 Skill"步骤中 |
...
| 来源 | 内容 | 本计划处理 |
|------|------|------------|
| `roadmap.md` V3 段（122–139 行） | `/agent-creator`、`/skill-creator`、`SkillRunner`、自建 Agent 走 SDK + anthropic 协议、Diff/部署 P2 择项 | 全部采纳；`/skill-creator` 定位为对话式生成 Skill，手动新增 / 上传 / 编辑 Skill 归设置页 Skill 管理 |
...
| # | 优先级 | 内容 | 涉及文件 |
|---|--------|------|----------|
| K1 | P0 | 状态机：用户描述想要的 Skill → LLM 以 tool call 生成 `SkillDraft` patch → 程序补问缺字段 → 预览 → 保存 | `lib/skills/skill-creator/state.ts`、`runner.ts`（新） |
| K2 | P0 | LLM 只负责生成 / 修正 `name`、`slug`、`description`、`body`；程序负责 `missing_fields`、下一张 Choice、状态推进与保存 | `lib/skills/skill-creator/prompts.ts`（新） |
| K3 | P0 | slug 唯一性校验 + 命名规范（小写字母 + 短横线）；冲突时用 Choice 卡引导换名 | 同上 |
| K4 | P1 | Skill 创建后立即在命令面板中可见（DB 写完后重新读取 registry） | `lib/skills/registry.ts` |
| K5 | P2 | Skill 版本号自动 +1；历史版本回看（V3.4 P2） | — |

> `/skill-creator` 不承担手动新增 / 上传 / 编辑 Skill。设置页 Skill 管理负责这些手动入口；聊天内 `/skill-creator` 只做"描述需求 → 生成 Skill 草稿 → 预览保存"。

### V3.4 ~ V3.6 · 自建 Agent 接入 + 群聊 UI + 自定义 Agent 设置页
...
- **流式体验差异化**（Q7 决策）：自建 Agent 走 SDK `includePartialMessages: true` 给**打字机效果**（逐 token 增量），V1.5 内置 Agent 是"按 turn 聚合"（整段出现）。两者体验不同，自建 Agent 气泡右上角加一个轻量"实时"角标（蓝色脉冲点）作为视觉提示，避免用户在两个 Agent 之间切换时产生"卡了"的错觉。视觉方案在 V3.5 UI 阶段定稿，UX 原型文件 `docs/design/prototypes/v3/self-built-agent-typewriter.html`（待补）
...
### V3.3 /skill-creator
- 用户用自然语言描述想要的 Skill 后，LLM 生成 `SkillDraft`，预览确认后 `skills` 表新增一行，`kind='user'`
- slug 不符合规范或冲突时，Choice 卡引导用户换名
- 创建后该 Skill 立即出现在 `/` 命令面板
- `/skill-creator` 不提供文件上传入口；手动上传 / 编辑 Skill 留给设置页
...
| V3.3 /skill-creator | 🟡 | LLM draft 生成 + 程序控状态机 + slug 规范 |
...
**目标**：跑通 `/skill-creator` 对话式生成流程：用户描述想要的 Skill，LLM 生成草稿，程序校验 / 补问，预览确认后保存，创建后立即可见。

**边界**：`/skill-creator` 只做对话式生成 Skill。手动新增、上传 markdown / yaml、编辑、删除等管理能力放到设置页 Skill 管理，不塞进斜杠命令流程。
...
| 子任务 | 内容 |
|--------|------|
| C0-1 | TypeScript 类型 + Zod schema：`SkillCreatorState`（`collecting` / `confirm_build` / `preview` / `saving` / `done` / `cancelled`）、`SkillCreatorEvent`、`SkillDraft`（name / slug / description / body）、`SkillCreatorExtractionResult`（summary / draft_patch / confidence / warnings）、`SkillSlugRule`（正则 `^[a-z][a-z0-9-]{1,30}$`） |
| C0-2 | 状态机转移表：用户输入 / Choice 回应 → LLM tool call 生成 `draft_patch` → 程序合并 draft、计算缺失字段、生成下一张 Choice；字段齐全 → `confirm_build` → `preview` |
| C0-3 | API endpoint：复用 `/api/messages` 与 `POST /api/interactions/:id/respond`；新增 `/api/skill-creator/:conversationId/session|save|regenerate|cancel`（形态参考 `/api/agent-creator/...`） |
| C0-4 | UI 组件 props：`SkillCreatorPreviewCard`（props: `draft: SkillDraft` / `status` / `error` / `onSave` / `onRegenerate` / `onCancel`）；不设计上传组件 |
...
| Commit | 内容 | 涉及文件 |
|--------|------|----------|
| C1 | K1 `state.ts` 状态机：`collecting` → `confirm_build` → `preview` → `saving` → `done` / `cancelled`；session 先用内存 Map，行为参考 `agent-creator` | `lib/skills/skill-creator/state.ts`（新） |
| C1 | K3 slug 命名规范：小写字母 + 短横线（`^[a-z][a-z0-9-]{1,30}$`），唯一性校验 | 同上 |
| C2 | LLM 生成：Anthropic / OpenAI compatible 均使用 tool call，返回 `draft_patch`；禁止依赖"文本里抽 JSON" | `lib/skills/skill-creator/prompts.ts`、`runner.ts` |
| C2 | 程序控流程：根据 `computeMissingFields(draft)` 决定下一张 Choice；LLM 不返回 `info_sufficient` / `next_question` | `lib/skills/skill-creator/runner.ts` |
| C3 | K4 创建后立即在命令面板可见：保存到 `skills` 表后下一次 `getSkills()` 可读到用户 Skill | `lib/skills/skill-creator/runner.ts` + `lib/skills/registry.ts` |
| C3 | 预览卡片：name / slug / description / body 摘要 / "保存" / "再改一下" / "取消" | 新组件 `SkillCreatorPreviewCard.tsx` |

**验收**：

- 用户用自然语言描述想要的 Skill，LLM 生成 `SkillDraft`，预览确认后 `skills` 表新增一行，`kind='user'`
- slug 不符合命名规范时给出明确错误
