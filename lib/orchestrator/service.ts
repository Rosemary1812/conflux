...
import { planOrchestratorRound } from "./planner";
...
import { evaluateTaskResult, shouldRevise, createRevisionTask } from "./evaluator";
...
async function handleClarify(
  ...
  db.insert(messages)
    .values({
      ...
      content: plan.clarificationQuestion || "能否补充更多细节，以便我更好地安排任务？",
      status: "done",
      createdAt: now + 1
    })
    .run();
...
async function executePlan(
  ...
  for (const task of plan.tasks) {
    ...
    if (!conversationAgentId) continue;
...
async function handleTaskCompleted(
  ...
  if (status === "done" && messageId) {
    const msg = db.select({ content: messages.content }).from(messages).where(eq(messages.id, messageId)).get();
    if (msg) {
      const evaluation = evaluateTaskResult(task as unknown as OrchestratorTaskRecord, msg.content);
      if (shouldRevise(evaluation)) {
        const revCount = revisionCount.get(taskId) || 0;
        if (revCount < 1) {
          revisionCount.set(taskId, revCount + 1);
          const revision = createRevisionTask(task as unknown as OrchestratorTaskRecord, evaluation.feedback || "需要修正");

          const revTaskId = crypto.randomUUID();
          db.insert(orchestratorTasks)
            .values({
              id: revTaskId,
              orchestratorRunId: task.orchestratorRunId,
              conversationId: task.conversationId,
              assigneeConversationAgentId: revision.assigneeConversationAgentId,
              roundId: `${task.roundId}-revise`,
              role: revision.role,
          ...
          // Immediately dispatch the revision since its dependency (the original task) is now done
...
async function finalizeOrchestratorRun(runId: string) {
  ...
  const summary = aggregateResults(tasks as unknown as OrchestratorTaskRecord[], messageMap);
  const now = Date.now();
